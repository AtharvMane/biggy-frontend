...
function RestaurantSide(props) {
  ...
  useEffect(() => {
    const continuousReq = setInterval(async () => {
      await axios
        .get(
          "https://biggy-backend.herokuapp.com/RestaurantSide/orders/getOrders",
          {
  ...
  const orderDone = async (element) => {
    await axios
      .post(
        "https://biggy-backend.herokuapp.com/RestaurantSide/orders/setOrders",
        element
      )
      .then((res) => {})
      .catch((err) => {
  ...
  const postMenu = async () => {
    await axios
      .post(
        "https://biggy-backend.herokuapp.com/RestaurantSide/menu/addMenu",
        { _id: params.id, addToMenu: name, cost: cost, image: imgPath },
        {
          ...
          <ul className={HotelMenuCss.menulist}>
            ...
            {addMenuForm && (
              <li className={HotelMenuCss.menulistItems}>
                ...
                >
                  <input
                    className={HotelMenuCss.inputtext}
                    type="text"
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      setimgPath(parseFloat(e.target.value) || "");
                    }}
                  ></input>
                ...
                return (
                  <li className={HotelMenuCss.menulistItems}>
                    <div className={HotelMenuCss.textClass}>
                      {e.cart.map((name) => {
                        return (
                          <p className={HotelMenuCss.menutext}>{name.name} </p>
                        );
                      })}
                    </div>
                    <button
